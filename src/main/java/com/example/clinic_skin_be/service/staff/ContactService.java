...
public class ContactService {
    ...
    public ConsultationAssignmentResponseDTO assignConsultant(ConsultationAssignmentDTO dto) {
        ...
        if (existingAssignment.isPresent()) {
            ConsultationAssignment assignment = existingAssignment.get();
            if (dto.getNote() != null) assignment.setNote(dto.getNote());
            throw new AssignmentAlreadyExistsException("Consultant already assigned to this contact");
        }

        // Tạo assignment mới
        ConsultationAssignment assignment = ConsultationAssignment.builder()
                .contact(contact)
